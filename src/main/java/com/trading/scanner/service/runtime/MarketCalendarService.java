...
public class MarketCalendarService {
    ...
    private static final String KEY_LAST_OFFICIAL_REFRESH_WEEK = "calendar.last.official.refresh.week";
    ...
    public List<ExchangeHoliday> holidays(
            LocalDate fromDate,
            LocalDate toDate) {

        return exchangeHolidayRepository
                .findByExchangeAndTradingDateBetweenOrderByTradingDateAsc(
                        EXCHANGE_NSE,
                        fromDate,
                        toDate);
    }
    ...
    public OfficialHolidayRefreshResult refreshFromOfficialNseSource() {
        ...
        parsedHolidays.putAll(
                parseOfficialNseTradingHolidayPayload(
                        fetchOfficialNseTradingHolidayPayload(
                                now.getYear())));
        ...
        for (Map.Entry<LocalDate, String> entry : parsedHolidays.entrySet()) {
            ...
            Optional<ExchangeHoliday> existing = exchangeHolidayRepository
                    .findByExchangeAndTradingDate(
                            EXCHANGE_NSE,
                            tradingDate);
            ...
            if (existing.isPresent()) {
                ...
                exchangeHolidayRepository.save(
    ...
    @Transactional
    public ScheduledCalendarRefreshResult refreshFromOfficialSourceIfDue(
        ...
        LocalTime configuredTime = LocalTime.parse(
        ...
        if (currentDate.getDayOfWeek() != DayOfWeek.MONDAY) {
            return new ScheduledCalendarRefreshResult(
                    ...
                    0,
                    0,
                    "Skipped calendar refresh because today is not Monday");
        }
        ...
        String refreshWeekKey = currentDate
                .with(TemporalAdjusters.previousOrSame(
                        DayOfWeek.MONDAY))
                .toString();

        String lastRefreshWeek = runtimeSettingService.getString(
                KEY_LAST_OFFICIAL_REFRESH_WEEK,
                "");

        if (refreshWeekKey.equals(lastRefreshWeek)) {
            return new ScheduledCalendarRefreshResult(
                    "NO_ACTION",
                    0,
                    0,
                    0,
                    "Skipped calendar refresh because this Monday was already refreshed");
        }
        ...
        try {
            ...
            runtimeSettingService.upsert(
                    ...
                    refreshWeekKey,
                    "DATE",
                    "Monday of the last successfully refreshed holiday week",
                    UPDATED_BY_SYSTEM);
    ...
    protected String fetchOfficialNseTradingHolidayPayload(
            int year) {
        ...
        try {
            String url = String.format(
                    runtimeAutomationProperties
                            .getCalendar()
                            .getOfficialNseTradingApiUrlTemplate(),
                    year);
            ...
            if (response.statusCode() < 200
                ...
                throw new IllegalStateException(
                        "Official NSE holiday API returned HTTP "
                                + response.statusCode());
            }
            ...
            throw new IllegalStateException(
                    "Failed to fetch official NSE trading holiday API",
                    ex);
        }
    }

    protected Map<LocalDate, String> parseOfficialNseTradingHolidayPayload(
            String payload) {

        try {
            JsonNode root = objectMapper.readTree(payload);

            String segment = runtimeAutomationProperties
                    .getCalendar()
                    .getOfficialNseTradingSegment();
            ...
            if (!segmentNode.isArray()
                ...
                throw new IllegalStateException(
                        "No exchange holidays found for NSE segment "
                                + segment);
            }
            ...
            for (int i = 0; i < segmentNode.size(); i++) {
                ...
                if (tradingDateNode == null
                        || descriptionNode == null) {

                    continue;
                }

                LocalDate tradingDate = parseNseTradingDate(
                        tradingDateNode.asText());

                String description = descriptionNode.asText().trim();

                holidays.putIfAbsent(
                        tradingDate,
                        description);
            }
            ...
            throw new IllegalStateException(
                    "Failed to parse official NSE trading holiday API payload",
                    ex);
        }
    }
    ...
    private LocalDate parseNseTradingDate(String rawDate) {
        if (rawDate.length() == 9) {
            return LocalDate.parse(
                    rawDate,
                    NSE_DATE_FORMAT_SHORT);
        }

        return LocalDate.parse(
                rawDate,
                NSE_DATE_FORMAT_LONG);
    }
